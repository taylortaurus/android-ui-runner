...
class tab_music(unittest.TestCase, BasePage):
    ...
    def test_01_music_show(self):
        ...
        self.assertTrue(self.d(text="test_xxx").exists(timeout=5),msg='新增的音频文件没有展示')
        self.screenshot()
    ...
    def test_02_music_more_rename(self):
        ...
        app.case_restart_check(text='MUSIC')
        self.d(text="MUSIC").click(timeout=5)
        if self.d(text="test_xxx").exists(timeout=5):
    ...
    def test_03_music_play(self):
        ...
        self.assertTrue(self.d(resourceId=res['com.app.xxxxxx:id/ivPlayOrPause']).exists(timeout=5),
                        msg='没有进入音频播放页')
        self.screenshot()

        ###### 暂停播放
        self.d(resourceId=res['com.app.xxxxxx:id/ivPlayOrPause']).click()  # 暂停
        time_before = self.d(resourceId=res['com.app.xxxxxx:id/tvStartTime']).get_text(timeout=5)
        self.d(resourceId=res['com.app.xxxxxx:id/ivPlayOrPause']).click()  # 播放
        time.sleep(5)
        time_after = self.d(resourceId=res['com.app.xxxxxx:id/tvStartTime']).get_text(timeout=5)
        self.assertNotEqual(time_before, time_after, msg='播放时间没有走动')

        ###### 下一首
        self.d(resourceId=res['com.app.xxxxxx:id/ivPlayNext']).click()  # 下一首
        action.exist_click(text='Exit')
        self.assertTrue(self.d(text="test_music1").exists(timeout=5), msg='切换下一首失败')

        ###### 上一首
        self.d(resourceId=res['com.app.xxxxxx:id/ivPlayPre']).click()
        action.exist_click(text='Exit')
        self.assertTrue(self.d(text="test_music0").exists(timeout=5), msg='切换上一首失败')

        ###### 底部播放栏
        action.exist_click(text='Exit')
        self.d(resourceId=res['com.app.xxxxxx:id/ivBack']).click()
        ...
        ###### 关闭播放栏
        self.d(resourceId=res['com.app.xxxxxx:id/ivClose']).click(timeout=5)
        self.screenshot()
