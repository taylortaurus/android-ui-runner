...
def check_devives():
    ...
    if method == 'SERVER':
        ...
        logger.info('Get available online devices from ATX-Server...')
        ...
        logger.info('Get available UDID devices %s from atxserver2...' % ReadConfig().get_server_udid())
        ...
        logger.info('Get available IP devices %s from config... ' % ReadConfig().get_devices_ip())
        devices = get_devices()
...
class Drivers:
    ...
    def _run_cases(run, cases):
        ...
        try:
            ...
            # 将logcat文件上传到报告
            base_page.d.shell('logcat *:E -d > /sdcard/logcat.log')
            time.sleep(1)
    ...
    def _run_maxim(run, cases, command, actions, widget_black):
        ...
        try:
            ...
            base_page.d.pull('/sdcard/crash-dump.log', os.path.join(path.get_path(), 'crash-dump.log'))
            base_page.d.pull('/sdcard/oom-traces.log', os.path.join(path.get_path(), 'oom-traces.log'))
